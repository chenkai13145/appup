  ...
  <div class="header">
   <!-- 顶部1 -->
    ...
    <div class="bulletin-wrapper">
			<img class="icon" v-if="headerdata.discounts2" :src="headerdata.discounts2[0].icon_url" />

			<span class="text" v-if="headerdata.discounts2">{{headerdata.discounts2[0].info}}</span>
			<div class="detail" v-if="headerdata.discounts2" @click="showActive">
				{{headerdata.discounts2.length}}个活动
    ...
    <div class="bg-wrapper" :style="head_pic_url">
			<!-- <img :src="headerdata.head_pic_url" /> -->
		</div>
      ...
      <div class="bulletin-detail" v-show="isShow">
          <div class="detail-wrapper">
            ...
            <div class="main-wrapper" :style="detail_bg">
              ...
              <h3 class="name">{{headerdata.name}}</h3>
              <!--评价 -->
              <div class="score">
                <!-- 星星传值 -->
                <small-app :score="headerdata.wm_poi_score"></small-app>
                <span>{{headerdata.wm_poi_score}}</span>
              </div>

              <p class="tip">
                {{headerdata.min_price_tip}} <i>|</i> {{headerdata.shipping_fee_tip}} <i>|</i> {{headerdata.delivery_time_tip}}
              </p>

              <p class="time">
                配送时间: {{headerdata.shipping_time}}
              </p>

              <div class="discounts" v-if="headerdata.discounts2">
                <p>
                  <img :src="headerdata.discounts2[0].icon_url" />
                  <span>{{headerdata.discounts2[0].info}}</span>
                </p>
              </div>
            </div>
...
<script>
import small from '../smalltu/smalltu.vue'
export default {
  data () {
    return {
       isShow:false
  ...
  computed:{
      head_bg(){
         return  `background-image: url(${this.headerdata.pic_url})`
       },
      head_pic_url(){
        return  `background-image: url(${this.headerdata.head_pic_url})`
      },
      detail_bg(){
        return  `background-image: url(${this.headerdata.detail_bg})`
      }
  },
  methods:{
    showActive(){
